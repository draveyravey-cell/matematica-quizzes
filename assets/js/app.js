...
const PREFIX = window.APP_CONFIG?.storagePrefix || "pp";
const SUBJECT = window.APP_CONFIG?.subjectName || "Português";
...
const STORAGE = { q: `${PREFIX}.f.q`, cat: `${PREFIX}.f.cat`, dif: `${PREFIX}.f.dif` };
const PAGER = { page: `${PREFIX}.pg.page`, size: `${PREFIX}.pg.size` };
const VIEW = { mode: `${PREFIX}.view.mode` };
const OV_KEYS = { data: `${PREFIX}.data.override`, enabled: `${PREFIX}.data.override.enabled` };
...
function renderLista() {
  ...
  pageItems.forEach((it, idx) => {
    ...
    h3.textContent = `Questão ${it.id ?? (start + idx + 1)} — ${it.tema || SUBJECT}`;
    ...
    btnStart.setAttribute("aria-label", `Responder questão ${it.id} — ${it.tema || it.categoria || SUBJECT}`);
